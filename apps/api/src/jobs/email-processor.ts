...
/**
 * Process a single email job: fetch it, send it via the project's configured
 * providers (with quota-aware fallback), and record the outcome. Extracted from
 * the worker handler so the exhaustion/requeue path is unit-testable without
 * standing up a queue.
 */
export async function processEmailJob(job: Job<SendEmailJobData>) {
  ...
  try {
    ...
    // Mark as sent with the provider message ID.
    //
    // Guarded on `sentAt` still being null so the campaign counter below is only
    // incremented by the run that actually stamped it. A job retried after the
    // provider accepted the message would otherwise count the same email twice.
    ...
    // Zero rows means another run already stamped this email -- the provider
    // accepted the message, then the job was retried. Everything below sends a
    // second signal for one delivery (a duplicate `email.sent` re-triggers
    // workflows), so stop here rather than replaying it. Finalization still runs:
    // this email is terminal either way, and the campaign must not be left stuck
    // in SENDING.
    if (marked.count === 0) {
